                     ...
                     double *x, double *y, double *z)
{
    ...
    for (i = 0, lambda = 380; lambda < 780.1; i++, lambda += 1) {
        double Me;

        Me = (*spec_intens)(lambda);
        X += Me * cie_colour_match[i/5][0];
        Y += Me * cie_colour_match[i/5][1];
        Z += Me * cie_colour_match[i/5][2];
    }
    XYZ = (X + Y + Z);
    *x = X / XYZ;
    *y = Y / XYZ;
...
int main()
{
    ...
    printf("// Wavelength to RGB values (conversion) Look-Up Table\n");
    printf("\nexport const wlToRgb = [\n");
    ...
    for (t = 380; t <= 780; t+= 1) {
        ...
        printf("[%.3f, %.3f, %.3f], // %.1f nm\n", 255.0 * r, 255.0 * g, 255.0 * b, t);
    }
    printf("];\n");
    return 0;
}
